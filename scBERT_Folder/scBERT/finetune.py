...
parser.add_argument("--seed", type=int, default=705, help='Random seed.')
...
parser.add_argument("--data_path", type=str, default='/workspace/scBERT_Folder/scBERT/gut_preprocessed_renamed.h5ad', help='Path of data for finetune.')
parser.add_argument("--model_path", type=str, default='/workspace/scBERT_Folder/scBERT/ckpts/finetune_best.pth', help='Path of pretrained model.')
parser.add_argument("--ckpt_dir", type=str, default='/workspace/scBERT_Folder/scBERT/ckpts/', help='Directory of checkpoint to save.')
...
sss = StratifiedShuffleSplit(n_splits=1, test_size=0.03, train_size=0.15, random_state=SEED)
...
trigger_times = 0
max_acc = 0.0
print("Running scBERT training on {} cells with validation dataset of {} cells".format(len(train_dataset), len(val_dataset)))
for i in range(1, EPOCHS+1):
    ...
    for index, (data, labels) in enumerate(train_loader):
        if index % 100 == 0:
            print("Processing cell no. {} - epoch {}".format(BATCH_SIZE*index, i))
        index += 1
    ...
    if i % VALIDATE_EVERY == 0:
        ...
        with torch.no_grad():
            ...
            if is_master:
                print(f'    ==  Epoch: {i} | Validation Loss: {val_loss:.6f} | Validation accuracy: {cur_acc:.6f}  ==')
